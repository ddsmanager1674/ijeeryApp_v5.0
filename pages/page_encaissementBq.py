...
class PageEncaissementBq(ctk.CTkToplevel):
    ...
    def generer_ticket_pdf(self, reference, categorie, montant, description, operateur):
        ...
        try:
            ...
            c.drawCentredString(x_centre, y, "TICKET DE ENCAISSEMENT BANQUE")
            y -= 7 * mm
    ...
    def enregistrer(self):
        """Enregistre l'encaissement bancaire avec l'utilisateur connecté."""
        if not self.conn or not self.cursor:
            return

        if self.bank_id is None:
            messagebox.showerror("Erreur", "Aucune banque sélectionnée. Impossible d'enregistrer l'encaissement.")
            return
        ...
        try:
            ...
            query = """
                INSERT INTO tb_encaissementbq (refpmt, idcc, mtpaye, observation, idtypeoperation, id_banque, datepmt, iduser)
...
if __name__ == "__main__":
    ...
    def open_encaissement_page_test():
        encaissement_page = PageEncaissementBq(root_for_test, username="TestUser", bank_id=1)

...
class PageRetardCredit(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        self.tree.tag_configure("critique", background="#FFCCCC", foreground="#8B0000")   # > 30 jours
        self.tree.tag_configure("warning",  background="#FFF0C0", foreground="#7A5F00")   # 15-30 jours
    ...
    def load_data(self, filter_text: str = ""):
        ...
        try:
            ...
            query = """
                ...
                WHERE v.idmode = 4                  -- Mode Crédit
                  AND pf.dateecheance < %s::date    -- Échéance dépassée
                GROUP BY c.nomcli, v.refvente, v.dateregistre,
                         v.totmtvente, pf.dateecheance
                HAVING (v.totmtvente - COALESCE(SUM(p.mtpaye), 0)) > 0  -- Solde non soldé
    ...
    def update_ui(self):
        ...
        for idx, row in enumerate(self.current_records):
            # Les 6 premières colonnes suffisent (total_paye et solde_restant
            # sont retournés par la requête mais utilisés uniquement côté SQL)
            client, num_fact, date_fact, montant, date_ech, nb_jours = row[:6]

            # Formatage des dates
            date_fact_str = date_fact.strftime("%d/%m/%Y") if date_fact else "-"
            date_ech_str  = date_ech.strftime("%d/%m/%Y")  if date_ech  else "-"

            # Choix du tag de couleur selon le nombre de jours
            ...
            total_mt += float(montant)

        self.total_montant.set(f"{total_mt:,.2f} Ar")

...
class PageEntree(ctk.CTkFrame):
    ...
    def open_recherche_article(self):
        ...
        # Stock consolidé incluant tb_entreedetail (nouveau mouvement +)
        QUERY_STOCK = """
        ...
        ent  AS (SELECT ed.idarticle,ed.idunite,ed.idmag,SUM(ed.qtentree)   AS q FROM tb_entreedetail ed WHERE ed.deleted=0 GROUP BY ed.idarticle,ed.idunite,ed.idmag),
        ...
        sor  AS (SELECT sd.idarticle,sd.idunite,sd.idmag,SUM(sd.qtsortie) AS q FROM tb_sortiedetail sd WHERE sd.deleted=0 GROUP BY sd.idarticle,sd.idunite,sd.idmag),
        ...
        mv AS (
            ...
            UNION ALL SELECT idarticle,idunite,idmag,q,'ent'  FROM ent
        ...
        solde AS (
            SELECT mv.idarticle, mv.idmag,
                SUM(CASE mv.t
                    ...
                    WHEN 'ent'   THEN  mv.q * COALESCE(uc.coeff_hierarchique,1)
            ...
            try:
                ...
                for idx, row in enumerate(cur.fetchall()):
                    tree.insert(
                        '',
                        'end',
                        values=(row[0], row[1], row[2] or "", row[3] or "", row[4] or "", self.formater_nombre(row[5])),

...
class PageFournisseur(ctk.CTkFrame):
    ...
    def load_fournisseur(self):
        ...
        try:
            ...
            self.all_frs_data = fournisseurs
            ...
            for idx, frs in enumerate(fournisseurs):
                ...
                try:
                    ...
                    dette_restante = 0
                dette_str = f" {self._formater_nombre(dette_restante)} Ar"
    ...
    def filter_fournisseurs(self, event=None):
        ...
        source = self.all_frs_data if search_query else self.all_frs_data
        idx = 0
        for frs in source:
    ...
    def open_frs_dette_details(self, idfrs):
        ...
        colonnes_dettes = ("ID", "Type", "Date", "Montant", "Montant Payé", "Solde Restant", "Statut")
        ...
        colonnes_pmt = ("ID", "Date Paiement", "Montant Payé", "Observation", "Utilisateur")
    ...
    def _fetch_frs_dettes(self, idfrs):
        """Retourne la liste unifiée des dettes d'un fournisseur (commandes + dettes manuelles)."""
        # Dettes issues des commandes fournisseur (montant de la commande vs ce qui a été payé)
        self.cursor.execute("""
            SELECT c.idcom,
                   'Dette Commande' as type,
                   c.refcom,
                   c.datecom,
                   COALESCE(c.totcmd, 0) as montant,
                   NULL::date as dateecheance
            FROM tb_commande c
            WHERE c.idfrs = %s AND c.deleted = 0
        """, (idfrs,))
        dettes_commande = self.cursor.fetchall()

        # Dettes manuelles (table tb_autredette si elle existe, sinon on utilise tb_pmtcom négatif)
        # On utilise une table dédiée tb_autredette similaire à tb_autrecreance côté client
        ...
        toutes_dettes = list(dettes_commande) + list(autres_dettes)
    ...
    def _compute_dette_status_fifo(self, idfrs):
        ...
        for dette in dettes_asc:
            dette_id, type_dette, ref, date_dette, montant_initial, _ = dette
